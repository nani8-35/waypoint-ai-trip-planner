...
2. Copy `.env.example` to `.env` and add `GEMINI_API_KEY` from Google AI Studio.
...
```text
React interface → Express API → Gemini Structured Outputs
       ↑               ↓
interactive UI ← validated JSON itinerary
```

1. The browser submits only the free-form trip request to `src/lib/api.js`.
2. `server/index.js` passes the request to Gemini with a strict JSON Schema and a 60-second timeout.
...
| Failed request | The interface explains the Gemini failure and offers retry. |
...
- `server/index.js`: Gemini integration and timeout handling.
