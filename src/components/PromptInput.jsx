...
export default function PromptInput({ value, onChange, onSubmit, loading }) {
  ...
  return <form className="prompt route-composer floating-composer" onSubmit={onSubmit}>
    ...
    <div className="floating-field"><span className="floating-mark" aria-hidden="true">✦</span><textarea id="trip-request" value={value} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} onChange={(e) => onChange(e.target.value)} placeholder="Ask Waypoint to plan a trip..." maxLength="5000" disabled={loading} /><button className="floating-submit" aria-label={loading ? 'Building your route' : 'Build route'} disabled={loading || !value.trim()}><span className="pulse-orb"><i /><b /></span><em>{loading ? 'Mapping' : 'Build plan'}</em></button></div>
